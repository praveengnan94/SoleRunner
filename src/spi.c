...
/* Enumeration for the Sleep Mode */
#define EM0 0
#define EM1 1
#define EM2 2
#define EM3 3
...
void blockSleepStatus(int minimumStatus);
void unblockSleepStatus(int minimumStatus);
void sleep(void);
void CmuSetup(void);
void GpioSetup(void);

/* This blocks sleep() from entering an energy mode below the given one */
void blockSleepStatus(int minimumStatus){
	/* Disable interrupts. */
	INT_Disable();
	sleep_counter[minimumStatus]++;
	/* Enable interrupts. */
	INT_Enable();
}

/* This unblocks sleep() from entering an energy mode below the given one */
void unblockSleepStatus(int minimumStatus){
	/* Disable interrupts. */
	INT_Disable();
	if(sleep_counter[minimumStatus]>0){
		sleep_counter[minimumStatus]--;
	}
	/* Enable interrupts. */
	INT_Enable();
...
/* This is a energy modes management module. The module allows the system to always sleep in the lowest possible energy mode  */
void sleep(void){
